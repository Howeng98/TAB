...
def patch_ex(ima_dest, ima_src=None, input_size=256, same=False, num_patches=1,
             mode=cv2.NORMAL_CLONE, width_bounds_pct=((0.05,0.2),(0.05,0.2)), min_object_pct=0.25, 
             min_overlap_pct=0.25, shift=True, label_mode='logistic-intensity', skip_background=None, tol=1, resize=True,
    ...
    if skip_background is not None:
        ...
        for background, threshold in skip_background:
            # print(ima_src.mean(axis=-1, keepdims=True), ima_src.mean(axis=-1, keepdims=True)-background)
            # print(ima_dest.mean(axis=-1, keepdims=True), ima_dest.mean(axis=-1, keepdims=True)-background)
        ...
        # print(src_object_mask.shape, dest_object_mask.shape)
    ...
    for i in range(num_patches):
        if i == 0 or np.random.randint(2) > 0:  # at least one patch
            patchex, ((_coor_min_dim1, _coor_max_dim1), (_coor_min_dim2, _coor_max_dim2)), patch_mask, ori_source_image = _patch_ex(
    ...
    # patchex = cv2.resize(patchex, (input_size, input_size))
    return patchex, label, ori_source_image
...
def _patch_ex(ima_dest, ima_src, dest_object_mask, src_object_mask, mode, label_mode, shift, resize, width_bounds_pct, 
    ...
    # ho-
    my_src = ima_src.copy()
    ori_source_image = cv2.rectangle(my_src, (coor_min_dim2, coor_min_dim1), (coor_max_dim2, coor_max_dim1 ), (255,0,0), 1)
    ...
    return patchex, ((coor_min_dim1, coor_max_dim1), (coor_min_dim2, coor_max_dim2)), patch_mask, ori_source_image