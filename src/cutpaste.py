...
class CutPasteNormal(CutPaste):
    ...
    """
    def __init__(self, input_size, area_ratio=[0.02,0.09], aspect_ratio=0.2, **kwags):
    ...
    def __call__(self, img):
        ...
        augmented_mask = torch.zeros((self.input_size, self.input_size, 1), dtype=torch.float32)
        augmented_mask[to_location_h:to_location_h + cut_h, to_location_w:to_location_w + cut_w] = 1.0
...
class CutPasteScar(CutPaste):
    ...
    """
    def __init__(self, input_size, width=[2,16], height=[10,25], rotation=[-45,45], **kwags):
    ...
    def __call__(self, img):
        ...
        augmented_mask = torch.zeros((self.input_size, self.input_size, 1), dtype=torch.float32)
        # augmented_mask[from_location_w:from_location_w+int(cut_w), from_location_h:from_location_h+int(cut_h)] = 1.0
